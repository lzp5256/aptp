...
class Article extends Base
{
    public function toCreate()
    {
        ...
        if(($check_res = $check_event->checkToCreateParams($param)) && $check_res['errCode'] != '200'){
            return json($check_res);
        }

        if(($handles_res = $handles_event->setData($check_res['data'])->handleToCreateRes()) && $handles_res['errCode'] != '200'){
